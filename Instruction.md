...
Удалённые репозитории представляют собой версии вашего проекта. Управление репозиториями включает в себя как умение добавлять новые, так и умение удалять устаревшие репозитории, а также умение управлять различными удалёнными ветками, объявлять их отслеживаемыми.
Чтобы начать работать с удаленными репозиториями необходимо:
1. Зайти на сайт GitHub.com
2. Авторизоваться
3. Создать аккаунт на GitHub
4. Создать локальный репозиторий

Создание нового репозитория осуществляется при помощи следуюших команды по порядку:
1. $ echo "# название файла" >> README.md
2. $ git init 
3. $ git add README.md
4. git commit -m "first commit"
Далее необходимо переименовать текущую ветку *master* в *main*, чтобы ветки в локальном и удалённом репозитории совпадали, т.к. GitHub по умолчанию создает ветку *main*. Для этого используется команда:
5. git branch –M main
6. git remote add origin https://github.com/mashaprokopeva/n.git
...
git pull
```
Данная команда "стягивает, выкачивает" все изменения из удаленного репозитория на свой компьютер. Делает слияние с локальным компьютером при помощи команды *`merge`*.
...
git remote –v
```
5. Создать удаленный репозиторий 
Для отправки данных с локального репозитория на удалённый используется команда :

```
 git push
 ```
6. Привязать удаленный репозиторий с локальным
...
```
Либо через контекстное меню "Открыть во встроенном терминале"  
Далее открываем папку во встроенном терминале и создаем новую ветку. 
```
git branch <имя ветки>
```
ВАЖНО: Терминал открывать только в той папке над которой работаешь. 
Далее вносим изменения  и добавляем файл в отслеживание, фиксируем изменения. Чтобы отправить изменения в удалённый репозиторий выполняем команду: 
```
